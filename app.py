...
def get_data_for_player(name, argument):
    ...
    if argument == 'show_games_one_master':
        ...
        cursor = conn.cursor()
        cursor.execute(
            f'SELECT schedule.photo FROM schedule WHERE id=%s', (master_id,))
        photo = cursor.fetchall()[0][0]
        cursor.close()
        conn.close()
...
def unsubscribe(player):
    ...
    cursor = conn.cursor()
    cursor.execute(
        f'SELECT schedule.title FROM schedule WHERE id=%s', (master_id,))
...
def check_games_player(player):
    ...
    if len(id_games):
        for id_game in id_games:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'DELETE FROM players WHERE id=%s ', (id_game,))
            conn.commit()
            cursor.close()
            conn.close()
...
def delete_game(name):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT schedule.title FROM schedule WHERE id=%s', (master_id,))
    ...
    cursor = conn.cursor()
    cursor.execute(
        'DELETE FROM schedule WHERE id=%s ', (master_id,))
    conn.commit()
    cursor.close()
    conn.close()
...
def check_games_master(master):
    ...
    if len(id_games):
        for id_game in id_games:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'DELETE FROM schedule WHERE id=%s ', (id_game,))
            conn.commit()
            cursor.close()
            conn.close()
